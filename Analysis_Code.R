...
# FTICR Analysis: ---------------------------------------------------------
#Load in metadata
metadata_fticr <- read_csv('Data/FTICR/FTICR_Meta_match.csv')

#Load in compound data:
compound_data <- read_csv('Data/FTICR/Report_processed_MolecFormulas.csv') %>%
  mutate('CompoundID' = paste0(Mass, '_', MolecularFormula)) 

#Make tidy:
compound_info <- compound_data %>% 
  pivot_longer(cols = metadata_fticr$SampleID, names_to = 'SampleID') %>%
  left_join(metadata_fticr) 

#At Day 0, find unique compounds between bog and fen:
unique_site_D0 <- compound_info %>%
  filter(Day == 'D0') %>%
  group_by(CompoundID, Site) %>%
  #For each compound, summarise if it is present at a specific site and day
  #n = number of samples it was detected in
  summarise(n = sum(value)) %>%
  ungroup() %>%
  #If the sample was detected in less than 2 samples, remove it
  mutate(n_adj = ifelse(n < 2, 0, 1)) %>%
  dplyr::select(-n) %>%
  pivot_wider(names_from = 'Site', values_from = 'n_adj') %>%
  #Remove compounds only detected in later timepoints:
  filter(Bog != 0 | Fen != 0) %>%
  mutate(unique = ifelse(Bog == 0 & Fen == 1, 'Fen',
                         ifelse(Bog == 1 & Fen == 0, 'Bog', 'Shared'))) %>%
  left_join(compound_data)

#How many unique at each site
unique_site_D0 %>%
  group_by(unique) %>%
  summarise(n = n())

#Set up the palette
pal_site_cap <- pal_site 
names(pal_site_cap) <- c('Bog', 'Fen')

nosc_data <- unique_site_D0 %>%
  filter(unique != 'Shared') 

#Plot the NOSC distribution of unique compounds:
...
unique_site_D0 %>%
  filter(unique != 'Shared') %>%
  wilcox.test(NOSC ~ unique, data = .) 

#Plot the feature classes:
comp_plot <- unique_site_D0 %>%
  mutate(Class_new = ifelse(Class == 'Protein' & N < 1, 'Lipid', 
                            ifelse(Class == 'Amino sugar' & N < 1, 'Carbohydrate', Class))) %>%
  group_by(unique, Class_new) %>%
  summarise(n_class = n()) %>%
  group_by(unique) %>%
  mutate(total = sum(n_class)) %>%
  ungroup() %>%
  mutate(p_class = n_class/total) %>%
  filter(unique != 'Shared')

comp_colors <- c('#EF476F', '#F8961E', '#06D6A0', '#118AB2', '#7400B8', 
                 '#C5D6D8','#BA97C3', '#414288', '#D1D646')
names(comp_colors) <- c('Amino sugar', 'Carbohydrate', 'Cond. HC', 'Lignin', 'Lipid', 
                        'Other', 'Protein', 'Tannin', 'Unsat. HC')
...
#Make the vanKrevelen Plot:
bog_vk <- unique_site_D0 %>%
  filter(unique == 'Bog')

fen_vk <- unique_site_D0 %>%
  filter(unique == 'Fen')

all_vk <- unique_site_D0 %>%
  filter(unique != 'Shared')
